// ============================================================
// IMPROVED: AUTOMATIC PROGRESS MONITORING
// ============================================================
...
function openArticlesWithDetector(count = 0) {
    ...
    function openNextBatch() {
                            ...
                            (function() {
                                ...
                                async function autoExtractAll() {
                                    ...
                                    updateProgress(
                                        ...
                                        \`Total collected: \${talStored.length} emails across all tabs\`,
                                        100
                                    );
        ...
        if (openedCount < totalToOpen) {
            ...
            console.log("📊 Auto-monitoring progress...");
            console.log("📥 After all tabs complete, run: downloadEmails()");
            
            // ============================================================
            // AUTO-START PROGRESS MONITOR
            // ============================================================
...
function showProgress() {
    ...
    console.log(`✅ Processed: ${progress.processedArtic|| 0}`);
    ...
    if (progress.tabsStatus) {
        ...
        for (const [tabId, status] of Object.entries(progress.tabsStatus)) {
            ...
            const emailInfo = status.lastEmail ? ` tus.lastEmail}` : '';
            const percentInfo = status.percent ? ` ${Math.round(status.percent)}%` : '';
            const detailInfo = status.detail ? ` - ${status.detail}` : '';
            console.log(`  ${tabIndex}. ${statusText}${percentInfo}${emailInfo}${detailInfo}`);
        }
    }
...
function startProgressMonitor() {
    ...
    console.log("   Press Ctrl+C or run stopProgressMonitor() to stop");
    console.log("");
...
function downloadEmails() {
    ...
    const vcfContacts = uniqueEmails.map((email) => {
        ...
        if (nameParts.length >= 2) {
            ...
            lastName = nameParts.slice(1)oin(' ');
...
function checkProgress() {
    ...
    console.log(`📊 Progress: ${unique.length} unique emails coled`);
    if (unique.length > 0) {
...
// ============================================================
console.log("\n" + "=".repeat(60));
console.log("📧 SCIENCE DIRECT EMAIL EXTRACTOR - AUTO-MONITOR");
console.log("=".repeat(60));
console.log("\n📌 Commands:");
console.log("  openArticlesWithDetector(3)  - Open 3 articles (auto-monitors)");
console.log("  shogress()              - Show detailed progress");
