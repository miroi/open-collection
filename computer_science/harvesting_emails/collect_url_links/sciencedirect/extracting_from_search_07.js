// ============================================================
// FINAL: SCIENCE DIRECT EMAIL EXTRACTOR - OPEN ALL ARTICLES
// ============================================================
// Adds openAllArticles() command to open ALL articles.
// ============================================================
                        ...
                        script.textContent = `
                            (function() {
                                ...
                                function reportEmailFound(email) {
                                    ...
                                    progress.tabsStatus[tabId].foundEmails = foundEmails;
                                    let totalEmails = 0;
                                    for (const key in progress.tabsStatus) {
                                        if (progress.tabsStatus[key].emailsFound) totalEmails += progress.tabsStatus[key].emailsFound;
                                    }
                                ...
                                function updateProgress(status, detail, percent, email) {
                                    ...
                                    if (emailListEl && foundEmails.length > 0) {
                                ...
                                function showComplete(emailsFound) {
                                    ...
                                    if (emailListEl && foundEmails.length > 0) {
                                        ...
                                        emailListEl.innerHTML = '📧 Found:<br>' + foundEmails.map(e => \`  \${e}\`).join('<br>');
                                    }
...
// ============================================================
// NEW: OPEN ALL ARTICLES
// ============================================================
function openAllArticles() {
    console.log("📌 Opening ALL articles from search results...");
    openArticlesWithDetector(0); // 0 means all articles
}
...
function showProgress() {
    ...
    if (progress.tabsStatus) {
        ...
        for (const [tabId, status] of Object.entries(progress.tabsStatus)) {
            ...
            const foundEmails = status.foundEmails || [];
            const emailList = foundEmails.length > 0 ? ` [${foundEmails.join(', ')}]` : '';
            console.log(`  ${tabIndex}. ${statusText}${percentInfo}${emailInfo}${emailList}`);
        }
    }
    console.log("=".repeat(60));
    return progress;
    ...
    if (progress.totalArticles > 0) {
        ...
        if (progress.tabsStatus) {
            ...
            for (const [tabId, status] of Object.entries(progress.tabsStatus)) {
                ...
                const found = status.foundEmails || [];
...
// ============================================================
console.log("\n" + "=".repeat(60));
console.log("📧 SCIENCE DIRECT EMAIL EXTRACTOR - OPEN ALL");
console.log("=".repeat(60));
console.log("\n📌 Commands:");
console.log("  openAllArticles()           - Open ALL articles (auto-download)");
