// ============================================================
// ISOLATED: SCIENCE DIRECT EMAIL EXTRACTOR
// ============================================================
// Each Chrome window gets its own unique session ID.
// Tabs open in the background without stealing focus.
// ============================================================
...
function openTabInBackground(url) {
    ...
    if (newTab) {
        try {
            // Immediately blur the new tab and focus current window
            newTab.blur();
            window.focus();
                                ...
                                function updateProgress(status, detail, percent, email) {
                                    ...
                                    if (emailListEl) {
                                        if (foundEmails.length > 0) {
                                            ...
                                            emailListEl.innerHTML = \`📧 \${foundEmails.length} email(s) found in this tab\`;
                                        } else {
                                ...
                                function showComplete(emailsFound) {
                                    ...
                                    if (emailListEl) {
                                        if (foundEmails.length > 0) {
                                            ...
                                            emailListEl.innerHTML = \`📧 Found \${foundEmails.length} email(s) in this tab\`;
                                        } else {
                                            emailListEl.style.display = 'none';
                                        }
                                    }
...
function showProgress() {
    ...
    console.log("=".repeat(60));
    return { tabsStatus, totalArticles, processedArticles, totalEmails };
}
...
function showProgressSummary() {
    const total = getStorage('totalArticles') || 0;
    const processed = getStorage('processedArticles') || 0;
    const emails = getStorage('totalEmails') || 0;
    const tabsStatus = getStorage('tabsStatus') || {};
    ...
    if (total > 0) {
        ...
        console.log(`📊 [${new Date().toLocaleTimeString()}] ${processed}/${total} articles (${percent}%) | 📧 ${emails} emails [${SESSION_ID.slice(0, 8)}]`);
        if (percentStr) {
...
// ============================================================
console.log("\n" + "=".repeat(60));
console.log("📧 SCIENCE DIRECT EMAIL EXTRACTOR - ISOLATED");
console.log("=".repeat(60));
console.log(`🆔 Session ID: ${SESSION_ID}`);
console.log(`💡 Tabs open in BACKGROUND - focus stays here!`);
console.log(`📁 This window's data is ISOLATED from other windows`);
...
console.log("  showProgress()              - Show progress");
