...
def get_rotated_molecule(symbol, bond_length, theta, phi):
    """
    Creates a diatomic molecule rotated by polar angle theta 
    and azimuthal angle phi.
    theta: angle from the surface normal (0 = vertical O-up, 180 = vertical Pb-up, 90 = flat)
    phi: rotation around the surface normal (0-360)
    """
    mol = Atoms(symbol, positions=[[0, 0, 0], [0, 0, bond_length]])
    
    # Rotate around Y by theta (polar tilt)
    mol.rotate(theta, 'y', center=(0, 0, 0))
    # Rotate around Z by phi (azimuthal orientation)
    mol.rotate(phi, 'z', center=(0, 0, 0))
    
    return mol

# Define search space
thetas = [0, 45, 90, 135, 180] # From vertical to flipped
phis = np.arange(0, 360, 90)    # Standard orientations
initial_height = 2.3

print(f"\n{'Site':<8} | {'Theta':<6} | {'Phi':<6} | {'E_ads (eV)':<10}")
print("-" * 45)
...
for site_name, (sx, sy) in manual_sites.items():
    for theta in thetas:
        for phi in phis:
            ...
            # Generate robustly rotated molecule
            # Pb is at [0,0,0], O is rotated relative to it
            mol = get_rotated_molecule('PbO', 2.3, theta, phi)
            
            # Use ASE's add_adsorbate to handle the height and periodic wrapping cleanly
            # This places the first atom of 'mol' (Pb) exactly at (sx, sy)
...
if results:
    ...
    print("-" * 45)
    print(f"Most Stable Configuration: {best_label} at {best_e:.4f} eV")
