...
class DiatomicAnalyzer:
    ...
    def _calculate_frequency_polyfit_fixed(self, atoms, n_points=11, range_factor=0.02):
        ...
        try:
            ...
            # Alternative formula: freq_cm1 = 5.308e-12 * sqrt(k_Nm / reduced_mass_amu)
            # where k is in N/m and mass in amu
            # freq_cm1 = 5.308e-12 * np.sqrt(k_Nm / reduced_mass_amu)
    ...
    def calculate_thermochemistry(self, atoms, freq_cm1, geometry='linear', 
        ...
        if atoms is None or freq_cm1 is None or np.isnan(freq_cm1):
            return 0.0, 0.0, 0.0
    ...
    def analyze_molecule(self, molecule_name, properties, config):
        ...
        temperature = general.get('temperature', 298.15)
        pressure = general.get('pressure', 101325)
    ...
    return {
        ...
        'advanced': {
            'vibrations': {
                'delta': 0.005,
                'nfree': 4,
                'method': 'numeric'  # Changed to 'numeric' for better accuracy
            },
            'optimization': {
                'algorithm': 'BFGS',
                'max_steps': 100,
...
def save_results(all_results, config):
    ...
    with open(csv_file, 'w') as f:
        ...
        f.write("Molecule,Calculator,d_eq(Å),D_e(eV),D_e(kJ/mol),freq(cm⁻¹),"
                "freq_method,H(kJ/mol),S(J/mol·K),G(kJ/mol)\n")
