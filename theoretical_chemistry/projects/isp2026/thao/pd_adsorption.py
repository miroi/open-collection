#!/usr/bin/env python
"""
Pd adsorption on Au(111) using ASE + EMT.

Computes adsorption energies for FCC, HCP, bridge, and ontop sites:

    E_ads = E_total - E_slab - E_atom

All reference energies are recomputed at runtime so the script is
...
def compute_E_atom():
    """Energy of an isolated Pd atom in a large box."""
...
# Site calculation
# ============================================================
def run_site(site, E_slab, E_atom):
    """Relax Pd/Au(111) at a given site; return a result dict."""
    slab = fcc111('Au', size=SLAB_SIZE, vacuum=VACUUM)
...
def main():
    ...
    print("=" * 68)
    print("Pd adsorption on Au(111)  --  ASE / EMT")
    print("=" * 68)
    ...
    E_slab, nsteps_slab, fmax_slab = compute_E_slab()
    E_atom = compute_E_atom()
