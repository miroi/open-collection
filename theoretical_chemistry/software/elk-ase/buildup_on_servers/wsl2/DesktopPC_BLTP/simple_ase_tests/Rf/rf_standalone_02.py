...
# --- MPI Settings ---
# Set number of MPI processes (change this value as needed)
nmpi = 4  # Number of MPI processes

# Set number of OpenMP threads per MPI process
nopenmp = 1  # Threads per MPI process
...
print(f"  MPI processes: {nmpi}")
print(f"  OpenMP threads per MPI process: {nopenmp}")
print(f"  Total threads: {nmpi * nopenmp}")
print()
...
os.environ['OMP_NUM_THREADS'] = str(nopenmp)
os.environ['MKL_NUM_THREADS'] = str(nopenmp)

# --- Run the calculation with MPI ---
print("\n" + "="*60)
print("Running Elk for Rutherfordium (Rf) with MPI")
print("="*60)
print(f"Using species file: ./rf_run/Rf.in")
print(f"k-point grid: 8x8x8")
print(f"Spin-orbit coupling: Enabled")
print(f"MPI processes: {nmpi}")
print(f"OpenMP threads per process: {nopenmp}")
print("="*60 + "\n")
...
try:
    ...
    if nmpi > 1:
        ...
        mpi_command = ['mpirun', '-np', str(nmpi), elk_exe]
        # Alternative: use 'mpiexec' if 'mpirun' is not available
        # mpi_command = ['mpiexec', '-n', str(nmpi), elk_exe]
    else:
        # Run without MPI
        mpi_command = [elk_exe]
    
    print(f"Running command: {' '.join(mpi_command)}")
    print()
                ...
                try:
                    ...
                    if len(parts) > 0:
                        ...
                        with open('energy.txt', 'w') as f:
                            ...
                            f.write(f"\nParallelization:\n")
                            f.write(f"  MPI processes: {nmpi}\n")
                            f.write(f"  OpenMP threads per process: {nopenmp}\n")
                            f.write(f"  Total threads: {nmpi * nopenmp}\n")
