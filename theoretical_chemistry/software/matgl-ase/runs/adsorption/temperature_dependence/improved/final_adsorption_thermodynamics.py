# final_adsorption_thermodynamics.py
"""
...
def adsorption_thermodynamics(T, E0, freq_list):
    """Calculate ΔH, ΔS, ΔG"""
    if T <= 0:
        return {'ΔH': E0, 'ΔS': 0.0, 'ΔG': E0, 'E_ads': E0}
...
ax1.set_title('Thermodynamic Functions')
...
ax3.axhline(y=E_ads_0, color='gray', linestyle='--', alpha=0.5, label=f'E_ads(0K) = {E_ads_0:.3f} eV')
ax3.axhline(y=0, color='black', linestyle=':', alpha=0.3)
ax3.set_xlabel('Temperature (K)')
ax3.set_ylabel('E_ads (eV)')
ax3.set_title('Adsorption Energy vs Temperature')
...
ax4.set_ylabel('K_eq')
ax4.set_title('Equilibrium Constant')
...
print(f"  R²: {r2:.4f}")
print(f"\n  ΔH = {ΔH_vant:.3f} eV = {ΔH_vant * 96.485:.2f} kJ/mol")
print(f"  ΔS = {ΔS_vant:.4f} eV/K = {ΔS_vant * N_A * 1.602e-19:.2f} J/(mol·K)")
print(f"  ΔG (298K) = {ΔG_vant_298:.3f} eV = {ΔG_vant_298 * 96.485:.2f} kJ/mol")
...
2. TEMPERATURE DEPENDENCE:
   ...
   • ΔG becomes LESS negative with T ✓
   • Entropy penalty increases with T
...
5. PHYSICAL INSIGHT:
   ...
   • Opposed by ENTROPY (loss of freedom)
   • Net: spontaneous at RT, less favorable at higher T
