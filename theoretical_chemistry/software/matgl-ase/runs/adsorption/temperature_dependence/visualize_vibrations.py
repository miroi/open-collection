...
modes = vib.get_modes()  # eigenvectors
...
# Classify modes based on displacement direction
mode_types = []
for i, freq in enumerate(frequencies):
    mode = modes[i][0]  # First atom (Hg)
    # Determine mode type based on dominant displacement
    if abs(mode[2]) > abs(mode[0]) and abs(mode[2]) > abs(mode[1]):
        mode_type = "Vertical"
    elif abs(mode[0]) > abs(mode[1]):
        mode_type = "Horizontal (x)"
    else:
        mode_type = "Horizontal (y)"
    
    mode_types.append(mode_type)
    print(f"{i+1:<8d} | {freq:15.1f} | {freq*0.029979:15.3f} | {mode_type:>15s}")
...
for mode_idx in range(min(3, len(frequencies))):
...
# Save animated GIF of each mode
try:
    import imageio
    from ase.io import write
    ...
    for mode_idx in range(min(3, len(frequencies))):
        ...
        for frame in frames:
            ...
            plt.tight_layout()
            plt.savefig(f'temp_frame_{mode_idx}_{step}.png', dpi=80)
            plt.close()
        ...
        filenames = sorted(glob.glob(f'temp_frame_{mode_idx}_*.png'))
...
surface_z = np.mean([pos[2] for pos in adslab.positions[:-1] if pos[2] < hg_z])
...
print(f"""
ZPE Corrections:
  • Harmonic ZPE: {0.5 * 4.135667696e-15 * 1e12 * sum(freq*0.029979):.4f} eV (calculated from frequencies)

Visualization Files Created:
  • relaxed_structure.xyz - Relaxed structure
  • vibrational_modes_3d.png - 3D mode visualization
  • vibration_mode_1.gif - Animation of Mode 1
  • vibration_mode_2.gif - Animation of Mode 2  
  • vibration_mode_3.gif - Animation of Mode 3
""")
...
print("  • Open vibration_mode_*.gif for animations")
