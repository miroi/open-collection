...
atoms = Atoms(
    ...
    positions=[ # positions in Angstrom 
        ...
        [0.0000000000000000, 1.8050000000000000, 1.8050000000000000],
        [1.8050000000000000, 0.0000000000000000, 1.8050000000000000],
        [1.8050000000000000, 1.8050000000000000, 0.0000000000000000]
    ],
    cell=[ # positions in Angstrom
        [3.6100000000000000, 0.0000000000000000, 0.0000000000000000],
        [0.0000000000000000, 3.6100000000000000, 0.0000000000000000],
        [0.0000000000000000, 0.0000000000000000, 3.6100000000000000]
    ],
    pbc=[True, True, True]
)
...
qe_bin = "/home/milias/work/software/qe/q-e-devel-official/build_openmpi"
...
# ==============================================
# 9. Quantum Capacitance Calculation
# ==============================================
print("\n[Phase C.] Calculating Quantum Capacitance per Unit Cell...", flush=True)

# 9.1. Extract Fermi level and DOS at E_F
fermi_level = nscf_calc.get_fermi_level()
print(f"  Fermi level: {fermi_level:.4f} eV", flush=True)

# Read DOS data
dos_data = np.loadtxt('total_dos.dat')
energy = dos_data[:, 0]
dos_states_per_cell_per_ev = dos_data[:, 1]  # states/eV/cell

# 9.2. Find DOS at Fermi level
index_ef = np.argmin(np.abs(energy - fermi_level))
dos_at_ef = dos_states_per_cell_per_ev[index_ef]

print(f"  DOS at E_F: {dos_at_ef:.6f} states/eV/cell", flush=True)

# 9.3. Calculate quantum capacitance per unit cell
e_charge = units._e  # Elementary charge in Coulombs
eV_to_J = units._e   # 1 eV = 1.602e-19 J (same numerical value as elementary charge)

Cq_per_cell = e_charge**2 * dos_at_ef / eV_to_J  # F/cell
# Units: (C²) × (states/eV) × (eV/J) = C²/J = F

print(f"  Quantum Capacitance per unit cell: {Cq_per_cell:.4e} F/cell", flush=True)

# 9.4. Save results
with open('quantum_capacitance.dat', 'w') as f:
    f.write("Quantum Capacitance Results - Per Unit Cell\n")
    f.write("==========================================\n")
    f.write(f"Fermi level: {fermi_level:.6f} eV\n")
    f.write(f"DOS at E_F: {dos_at_ef:.6f} states/eV/cell\n")
    f.write(f"Quantum Capacitance: {Cq_per_cell:.6e} F/cell\n")

print("  Results saved to quantum_capacitance.dat", flush=True)
...
print("- Quantum capacitance data saved to quantum_capacitance.dat", flush=True)
