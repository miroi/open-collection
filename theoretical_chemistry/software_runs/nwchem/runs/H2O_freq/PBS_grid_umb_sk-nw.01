...
#mpirun -np $NPROCS_PBS  $NWCHEM_EXECUTABLE  $PBS_O_WORKDIR/N2.scf_geomopt_sto2g.nw  > $PBS_O_WORKDIR/N2.scf_geomopt.out
mpirun -np $NPROCS_PBS  $NWCHEM_EXECUTABLE  $PBS_O_WORKDIR/h2o_freq_tasks.nw > $PBS_O_WORKDIR/h2o_freq_tasks.out

echo -e "\n Working files in scratch dir $TMPDIR:"
ls -lt $TMPDIR 
du -h -s $TMPDIR
cd .. ; echo -e "just for control -  pwd=\c";pwd
echo -e "Deleting scratch directory:"; /bin/rm -r $TMPDIR
echo -e "Remaining content ls -lt /mnt/local/$USER/ : "; ls -lt /mnt/local/$USER/

echo -e "returning to $PBS_O_WORKDIR"
cd $PBS_O_WORKDIR
